...
class ShareViewModel extends declared(Accessor) {
	...
	shareItems: Collection<ShareItem> = new ShareItemCollection([ FACEBOOK_ITEM, TWITTER_ITEM ]);
	...
	private _generateShareUrlParams(point: Point): string {
		...
		const defaultObjectId = this.defaultObjectId !== null ? `&defaultObjectId=${this.defaultObjectId}` : '';

		const attachmentIndex = this.attachmentIndex !== null ? `&attachmentIndex=${this.attachmentIndex}` : '';
